...
def add_results(file_path, gen, cl, wsi, num_trial, condition, results):
    ...
    if set(cols).issubset(df.columns):
        ...
        results.append([num_trial, wsi, gen, condition, cl, acc, tp, fn, tn, fp, precision, recall, f1])  # Add trial and WSI ID for reference
    else:
...
df = pd.DataFrame(results, columns=['Trial', 'WSI', 'Gen', 'Condition', 'Class', 'Accuracy', 'TP', 'FN', 'TN', 'FP', 'Precision', 'Recall', 'F1'])
...
merged = pd.merge(df_tumor, df_normal[['Trial', 'WSI', 'Gen', 'Condition', 'N_F1', 'N_Precision', 'N_Recall', 'N_TP', 'N_FN', 'N_TN', 'N_FP']], on=['Trial', 'WSI', 'Gen', 'Condition'], how='left')
merged = pd.merge(merged, df_fib[['Trial', 'WSI', 'Gen', 'Condition', 'F_F1', 'F_Precision', 'F_Recall', 'F_TP', 'F_FN', 'F_TN', 'F_FP']], on=['Trial', 'WSI', 'Gen', 'Condition'], how='left')
...
# --- Final columns ---
custom_order = [
    'Trial', 'WSI', 'Gen', 'Condition', 'Accuracy',
    'T_TP', 'T_FN', 'T_TN', 'T_FP', 
    'N_TP', 'N_FN', 'N_TN', 'N_FP', 
    'F_TP', 'F_FN', 'F_TN', 'F_FP',
